...
export class BudgetValidator {
  static validate(
    ...
    if (actual.frontend.bundleSize.totalSize) {
      const bundleViolation = this.validateMetric(
        'bundle',
        'totalSize',
      ...
      if (
        ...
        const chunkViolation = this.validateMetric(
          'bundle',
          'maxChunkSize',
    ...
    // Calculate score
    const score = this.calculateBudgetScore(violations, budget);
  ...
  private static validateMetric(
    ...
    const severity = this.getViolationSeverity(percentageOver);
    ...
    return {
      ...
      percentageOver,
      severity,
      recommendation: this.getRecommendation(type, metric, percentageOver),
    };
  }
